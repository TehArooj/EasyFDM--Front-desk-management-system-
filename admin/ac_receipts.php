<?php 
  ...
  // Select receipt charge codes for combo.
  $query = "SELECT CHARGE_ID, DESCRIPTION FROM charge_code WHERE ACTIVE = 1 AND FOR_RECEIPT = 1 ORDER BY CHARGE_ID;";
  $charge_codes = mysqli_query($db, $query);

  // Select grid data.
  $query = "SELECT RECEIPT_ID,RECEIPT_DATE,T2.BOOKING_NO,T2.NAME,T3.DESCRIPTION,AMOUNT, NARRATION
            FROM receipt T1,BOOKING T2,CHARGE_CODE T3
  ...
  if ( isset($_POST['btn_add']) )
  {
    ...
    // Add to Database
    if (count($errors) == 0) {
        $query = "INSERT INTO receipt (BOOKING_ID, CHARGE_ID, RECEIPT_DATE, AMOUNT, NARRATION, CREATED_BY, CREATED_ON, UPDATED_BY, UPDATED_ON) 
  ...
  if(isset($_GET['action']) && isset($_GET['recid']) && htmlspecialchars($_GET['action']) == "edit" ) 
  {
    ...
    $query = "SELECT RECEIPT_ID, BOOKING_ID, CHARGE_ID, RECEIPT_DATE, AMOUNT, NARRATION FROM receipt WHERE RECEIPT_ID = $recid";
  ...
  {
    $recid = mysqli_real_escape_string($db, $_GET['recid']);
    $query = "DELETE FROM receipt WHERE RECEIPT_ID = $recid";
    $pass = mysqli_query($db, $query);
